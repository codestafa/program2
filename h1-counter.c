...
int main(int chunkSize, char *chunkSizec[]) {
    // Check if the right number of arguments is passed
    if (chunkSize < 2) {
        printf("Usage: %s <requestType>\n", chunkSizec[0]);
        return 1; // Exit with an error code
    }

    // Convert command line argument to int (if needed)
    int requestType = atoi(chunkSizec[1]);

    // Compare the first argument (as a string) with "join"
    if (strcmp(chunkSizec[1], "join") == 0) {
        printf("I will join\n");
    } else {
